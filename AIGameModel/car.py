...
class Car:
    ...
    def draw_sensors(self, win):
        # [front, left-front, left, right-front, right] sensors pointing
        sensor_angles = [90, 45, 0, 135, 180]
        center_x = int(self.x + self.img.get_width() // 2)
        center_y = int(self.y + self.img.get_height() // 2)

        for angle in sensor_angles:
            length = self.cast_ray(angle)
            end_x = int(center_x + math.cos(math.radians(self.angle + angle)) * length)
            end_y = int(center_y - math.sin(math.radians(self.angle + angle)) * length)
            pygame.draw.line(win, (255, 0, 0), (center_x, center_y), (end_x, end_y), 2)
            pygame.draw.circle(win, (0, 255, 0), (end_x, end_y), 2)
